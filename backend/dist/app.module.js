        ...
        imports: [
            ...
            typeorm_1.TypeOrmModule.forRootAsync({
                ...
                useFactory: (config) => {
                    ...
                    const host = config.get('MYSQLHOST') || config.get('DB_HOST', 'localhost');
                    const port = parseInt(config.get('MYSQLPORT') || config.get('DB_PORT', '3306'), 10);
                    const username = config.get('MYSQLUSER') || config.get('DB_USER', 'root');
                    const database = config.get('MYSQLDATABASE') || config.get('DB_NAME', 'dygas_db');
                    ...
                    return {
                        ...
                        port,
                        username,
                        password: config.get('MYSQLPASSWORD') || config.get('DB_PASS', ''),
                        database,
